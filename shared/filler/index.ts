import type { Profile, FillSummary, LlmFieldConfidence } from '../types';
...
export interface FillOptions {
  ...
  /**
   * FR-5.3 — invoked for every control the heuristics left at `low` / `none`,
   * i.e. exactly the candidates for LLM classification. Fields that *were*
   * recognised (including ones skipped because the profile value is empty) are
   * never reported here: they are already decided.
   *
   * Collecting them during the pass costs nothing; re-enumerating and re-scoring
   * the page afterwards would double the work and could disagree with what was
   * just filled.
   */
...
export function fillPage(profile: Profile, opts: FillOptions = {}): FillSummary {
  ...
  // Defence in depth for P0-4: even if a credential field slips through
  // enumeration, it is never scored and never written to.
  ...
  const summary: FillSummary = {
    ...
    // highlight. Keeps `high + medium + unrecognized + aiQuestions + fileInputs
    // === total` true, which was not the case before.
  ...
  // File inputs are highlighted, never filled (FR-2.5)
  ...
  for (const el of elements) {
    ...
    // Remember the cover-letter field even when we have no text for it yet:
    // "Generate motivation → Insert" needs a target later (P1-12).
    ...
    if (writeInto(el, value)) {
      countMatch(summary, match.confidence);
...
// ─── FR-5.3 — second pass: LLM classification of what heuristics missed ───────
...
export interface LlmPassOptions {
  /**
   * The FR-5.3 opt-in, read from `AppSettings.llmFieldClassification`.
   * Fail-closed: anything other than a literal `true` sends nothing at all.
   */
  enabled: boolean;
...
/**
 * FR-5.3 — the second pass.
 *
 * Runs *after* `fillPage` has already filled and highlighted everything it could
 * recognise, over the fingerprints that pass reported through
 * `FillOptions.onUnresolved`. It is a plain async function with no timers of its
 * own: the caller starts it without awaiting, so heuristic filling is never held
 * up by a network request.
 *
 * Every failure mode is silent (FR-5.3, requirement 6): no key, no network, a
 * timeout, a hallucinated answer, an index that does not exist — all of them end
 * with fields left exactly as the heuristics left them. This is an optional
 * convenience; it never gets to interrupt anyone.
 *
 * A filled field is *always* highlighted {@link LLM_FIELD_CONFIDENCE} — amber,
 * "check this" — never `high`. See {@link LlmFieldConfidence}.
 */
export async function classifyUnresolvedFields(
  ...
  try {
    ...
    return nothingClassified(batch.payload.length); // FR-5.3: silent
  }
  ...
  for (const [key, fieldType] of Object.entries(classifications)) {
    ...
    // Defence in depth (P0-4): a model instruction is not a reason to write
    // profile data into a credential field.
    if (isSensitiveControl(el) || isInsideAuthForm(el)) continue;

    const value = resolveValue(fieldType, profile, options.coverLetterText ?? '');
    if (!value) continue; // unknown type, or the profile has nothing to offer
    if (!writeInto(el, value)) continue;

    if (fieldType === 'coverLetter') rememberRecognizedCoverField(el);
